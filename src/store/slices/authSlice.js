...
export const addCompanyAction=(name,description,bin,address,contactEmail,contactPhone)=>async(dispatch)=>{
    ...
    export const UpdateTitleNumberAction=(formData)=>async(dispatch)=>{
      console.log('UpdateTitleNumberAction started')
      
    
      const token = localStorage.getItem("token");
      for (const value of formData.values()) {
       
        console.log("Pass array of refresh = ", value)
    }
      ...
      try {
        const response = await axios.put(
          `${END_POINT}/api/editalltitlenumbers`,formData,{
            headers: {
              // 'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json', // Set the content type to JSON
            },
    ...
    export const UpdateSubTitleAction=(formData)=>async(dispatch)=>{
      console.log('addFullProfileDataAction started')
      ...
      try {
        const response = await axios.put(
          `${END_POINT}/api/editsubtitle`,formData,{
            headers: {
              // 'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json', // Set the content type to JSON
            },
